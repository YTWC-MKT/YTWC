...
export default function Blog() {
  return (
    <>
      <div className="h-20" />
      <PageBreadcrumb items={[{ label: "Home", to: "/" }, { label: "Blog" }]} />
            ...
            {BLOG_POSTS.map((p) => (
              <Link key={p.slug} to={`/blog/${p.slug}`} data-testid={`blog-card-${p.slug}`} className="group block bg-[#111111] border border-white/10 hover:border-gold/40 transition-colors">
                <div className="aspect-video overflow-hidden">
                  <img src={p.img} alt={`${p.title} — YTWC blog article`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <div className="p-7">
                  <div className="flex items-center gap-3 text-xs uppercase tracking-wider text-gold mb-3">
                    <span>{p.cat}</span><span className="text-white/20">·</span><span className="text-white/40">{p.date}</span>
                  </div>
